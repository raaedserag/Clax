...
router.get("/get-balance", async (req, res) => {
    ...
    if(userQuery.success && !userQuery.result) return res.status(400).send("user not found")
...
router.get("/get-cards", async (req, res) => {
    ...
    if(! (userObject.success && userObject.result)) return res.status(500).send("Failed to implement");

    // Retreive cards info
    const cardsQuery = await stripeController.getCards(userObject.result.stripeId)
    // If retreiving cards failed
    if(! (cardsQuery.success && cardsQuery.result)) return res.status(500).send("Failed to implement");
    
    // IF ALL IS WELL
    res.status(200)
    .send(_.map(cardsQuery.result.data, _.partialRight(_.pick, ['id', 'exp_year', 'exp_month', 'last4'])))
});
...
router.post("/add-card", async (req, res) => {
    ...
    if(! (createCardToken.success && createCardToken.result)) return res.status(500).send("Failed to implement");
    ...
    const createSourceToken = await stripeController.createSource(userObject.result.stripeId, createCardToken.result.id)
    // If generating source failed 
    if(! (createSourceToken.success && createSourceToken.result)) return res.status(500).send("Failed to implement");
...
router.post("/charge-balance", async (req, res) => {
    ...
    // Creating charge
    const creatingCharge = await stripeController.chargeBalance(req.body.source, req.body.amount)
    // If creating charge failed
    if(! (creatingCharge.success && creatingCharge.result)) return res.status(500).send("Failed to implement");

...
const cardSchema = Joi.object().keys({
    ...
    exp_month: Joi.string()
      ...
      .custom((value, helpers) => {
        ...
        if(isNaN(value) || !Number.isInteger(value) || value<1 || value>12) return helpers.error('any.invalid');
        // else
        return value
    }, 'month Validation'),
    exp_year: Joi.string()
      ...
      .custom((value, helpers) => {
        value = parseFloat(value);
        if(isNaN(value) || !Number.isInteger(value) || value<2000) return helpers.error('any.invalid');
        // else
        return value
    }, 'year Validation'),
    cvc: Joi.number()
      ...
      .custom((value, helpers) => {
        value = parseFloat(value);
        if(isNaN(value) || !Number.isInteger(value) || value<100 || value>9999) return helpers.error('any.invalid');
        // else
        return value
    }, 'cvc Validation'),
